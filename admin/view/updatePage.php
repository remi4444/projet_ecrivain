...
foreach($chapterElement as $chapter)
    {?>
<form action="indexAdmin.php?action=updateChapter&amp;id=<?=$chapter->getId()?>" method = "post" class="reponsive_width jumbotron ">
            <label class="text-danger">>Entrez le numéro de chapitre : </label><input class="width_case size_form_admin" type = "text" name = "chapter_number" value = "<?= $chapter->getChapterNumber()?>"><br>
